  ...
  for (size_t i=0;i!=live_clusters.size();i++){
	...
	if (!flag_dead){
      ...
      if (((num_unique[0] <= 0.1 * (num_total_points - num_dead[0]) || num_unique[0] <= 0.1 * num_total_points && num_unique[0] <= 8) &&
	   ...
	   num_unique[0]==0 && num_unique[1]==0 && num_unique[2] < 0.18 * num_total_points ||
	   num_unique[0]==0 && num_unique[2]==0 && num_unique[1] < 0.18 * num_total_points ||
	   num_unique[2]==0 && num_unique[1]==0 && num_unique[0] < 0.18 * num_total_points ||
	   ...
	   (num_unique[0] + num_unique[1] + num_unique[2])  <= 200 ){
	flag_save = false;
