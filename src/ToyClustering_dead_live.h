  ...
  double time_slice_width = mp.get_ts_width();
   ...
   // debug
	for (auto it = dead_live_cluster_mapping.begin(); it!=dead_live_cluster_mapping.end(); it++){
		if (it->second.size()>1){
			std::cout << "Xin: " << it->second.size() << " " ;
			int ncount = 0;
			int ncount1 = 0;
			PR3DCluster* temp_cluster = 0;
			for (auto it1 = it->second.begin(); it1!=it->second.end(); it1++){
				std::cout << cluster_length_map[*it1]/units::cm << " ";
				if (cluster_length_map[*it1] > 100 *units::cm) ncount ++;
				if (cluster_length_map[*it1] > 200*units::cm) {
					ncount1 ++;
					temp_cluster = *it1;
				}
			}
			std::cout << std::endl;
			if (ncount == 2 && ncount1 == 1){
				//std::cout << it->first->get_num_mcells() << " " << it->first->get_num_time_slices() << std::endl;
				/*
				SMGCSelection& mcells = it->first->get_mcells();
				std::map<SlimMergeGeomCell*, std::set<int>>& cell_times_set_map = it->first->get_cell_times_set_map();
				for (auto it1 = mcells.begin(); it1!=mcells.end(); it1++){
					std::cout << "U: " << (*it1)->get_uwires().front()->index() << " " << (*it1)->get_uwires().back()->index()
							  << " V: " <<  (*it1)->get_vwires().front()->index() << " " << (*it1)->get_vwires().back()->index()
						      << " W: " <<  (*it1)->get_wwires().front()->index() << " " << (*it1)->get_wwires().back()->index()
						      << " T: "  << *cell_times_set_map[*it1].begin() << " " << *cell_times_set_map[*it1].rbegin()
							  << std::endl;
				}
				*/
				std::map<int,SMGCSet>& time_cells_set_map = temp_cluster->get_time_cells_set_map();
				SlimMergeGeomCell *blob = (*(time_cells_set_map.begin()->second.rbegin()));
				std::map<SlimMergeGeomCell*, std::set<int>>& cell_times_set_map = temp_cluster->get_cell_times_set_map();
      ...
      for (size_t i=0; i!= connected_live_clusters.size(); i++){
		...
		cluster_connected_dead.insert(cluster_1);
      	...
      	for (size_t j=i+1;j<connected_live_clusters.size(); j++){
			...
			// std::cout << "test: " << cluster_length_map[cluster_1]/units::cm << " " << cluster_length_map[cluster_2]/units::cm << std::endl;
	    ...
	    if (dis < 60*units::cm){
	      ...
	      //		std::cout << "xin4 " << length_1/units::cm << " " << length_2/units::cm << std::endl;
					...
					// dir1.X() << ", " << dir1.Y() << ", " << dir1.Z()  << ") (" << dir3.X() << ", " << dir3.Y() << ", " << dir3.Z() 
					// << ") " << angle_diff1 << " " << angle_diff2 << " " << angle_diff3 << std::endl;
	      //}
		...
		if (length_1 <= 12*units::cm && length_2 <=12*units::cm){
	      ...
	      if (flag_merge){
			...
			// std::cout << "success: " << length_1/units::cm << " " << length_2/units::cm << std::endl;
	      }
	    }
