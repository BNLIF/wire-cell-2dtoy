...
bool WireCell2dToy::Clustering_3rd_round(WireCell::PR3DCluster *cluster1, WireCell::PR3DCluster *cluster2, double length_1, double length_2, double length_cut){
  ...
  // if (cluster1->get_cluster_id()==87 || cluster2->get_cluster_id()==87)
  ...
  if (dis < length_cut && (length_2 >=12*units::cm || length_1 >=12*units::cm)){
    ...
    int num_tp1 = cluster1->get_num_points();
...
bool WireCell2dToy::Clustering_2nd_round(WireCell::PR3DCluster *cluster1, WireCell::PR3DCluster *cluster2, double length_1, double length_2, double length_cut){
  ...
  // if (cluster1->get_cluster_id()==34 || cluster2->get_cluster_id()==34)
...
bool WireCell2dToy::Clustering_1st_round(WireCell::PR3DCluster *cluster1, WireCell::PR3DCluster *cluster2, double length_1, double length_2, double length_cut, bool flag_enable_extend){
  ...
  if (dis < length_cut){
    ...
    if (flag_dir){
      ...
      double angle_diff2 = (3.1415926-dir1.Angle(dir3))/3.1415926*180.;
      double angle_diff2_1 = (3.1415926-dir1_1.Angle(dir3_1))/3.1415926*180.;
      double angle_diff2_2 = (3.1415926-dir1.Angle(dir3_1))/3.1415926*180.;
    ...
    if (connected_live_clusters.size()>1){
      ...
      for (size_t i=0; i!= connected_live_clusters.size(); i++){
	    ...
	    bool flag_para =false;
	    {
	      ...
	      TVector3 drift_dir(1,0,0);
	      double angle1 = dir1.Angle(drift_dir);
	      double angle2 = dir2.Angle(drift_dir);
	      double angle3 = dir3.Angle(drift_dir);
	      if (fabs(angle1-3.1415926/2.)<5/180.*3.1415926 &&
		  fabs(angle2-3.1415926/2.)<5/180.*3.1415926 &&
		  fabs(angle3-3.1415926/2.)<5/180.*3.1415926 ){
		if (  dis < 25*units::cm)
	    ...
	    if (!flag_merge){
	      if (length_1 <= 12*units::cm && length_2 <=12*units::cm){
	    	// both are short
	    	if ((dis <= 3*units::cm|| flag_para) && (angle_diff1 <= 45 || angle_diff2 <=45) ||
	      	...
	      	// one is short
	      	if ((dis <= 3*units::cm|| flag_para)  && (angle_diff2 <= 45 || angle_diff2<=45) 
	    ...
	    // if (cluster_1->get_cluster_id()==24 || cluster_2->get_cluster_id()==24 ||
	    // 	cluster_1->get_cluster_id()==19 || cluster_2->get_cluster_id()==19 )
	    //   std::cout << cluster_1->get_cluster_id() << " " << cluster_2->get_cluster_id() << " " << dis/units::cm << " "  << cluster_length_vec.at(i)/units::cm << " " << cluster_length_vec.at(j)/units::cm << " " << angle_diff1 << " " << angle_diff2 << " " << flag_merge<< std::endl;
